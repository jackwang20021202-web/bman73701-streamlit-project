...
def init_state():
    ...
    if "df_full" not in st.session_state:
        # try load default data if exists
...
OUT_TASK4 = Path("outputs") / "task4"   # 你也可以改成你项目里已有的 output 路径
...
def page_task4():
    st.header("Task 4 — AED exploration")
    ...
    tab1, tab2 = st.tabs(["Task 4.1 Main features", "Task 4.2 Relationships"])

    # -----------------------
    # 4.1
    # -----------------------
    with tab1:
        st.subheader("Task 4.1 — Summary + figures")
        ...
        with c2:
            if st.button("Show summary table (in-app)", key="show_t4_summary"):
                try:
                    ...
                    st.caption("Also exported to outputs/task4/Task4_summary_table.xlsx (+ CSV).")
                except Exception as e:
        ...
        with c_dl1:
            if xlsx_path.exists():
                ...
                st.caption("Excel file not generated yet.")
        ...
        with c_dl2:
            if csv_path.exists():
                ...
                st.caption("CSV file not generated yet.")

        st.divider()
        st.subheader("Figures (choose what to display)")
        show_selected_images(FIGS_41, OUT_TASK4, key_prefix="t41")

    # -----------------------
    # 4.2
    # -----------------------
    with tab2:
        st.subheader("Task 4.2 — Interesting relationships")

        # 用 sidebar 的 sample_seed 作为 jitter seed（不影响抽样）
        ...
        st.divider()
        st.subheader("Figures (choose what to display)")
        show_selected_images(FIGS_42, OUT_TASK4, key_prefix="t42")


OUT_TASK5 = Path("outputs") / "task5"
OUT_TASK5.mkdir(parents=True, exist_ok=True)


def page_task5():
    st.header("Task 5 — Breach factors")
    ...
    # -------- 5.1 Overall
    st.subheader("5.1 Overall breach rate")
    ...
    tabA, tabB, tabC = st.tabs(["5.2 Categorical factors", "5.3 Numeric factors", "5.4 Output pack"])

    # -------- 5.2 Categorical
    with tabA:
        st.subheader("Categorical factor → breach rate")
        ...
        cat = st.selectbox("Choose categorical variable", cat_vars, key="t5_cat")

        show_opts = ["Table", "Bar chart", "Download CSV"]
        picked = st.multiselect("Choose outputs", show_opts, default=["Table"], key="t5_cat_show")
    ...
    # -------- 5.3 Numeric
    with tabB:
        st.subheader("Numeric factor → compare Breach vs No breach")
        ...
        num = st.selectbox("Choose numeric variable", num_vars, key="t5_num")

        show_opts = ["Summary table", "Boxplot", "Download CSV"]
        picked = st.multiselect("Choose outputs", show_opts, default=["Summary table"], key="t5_num_show")

        tbl2 = numeric_summary_by_breach(df, num)

        if "Summary table" in picked:
            st.dataframe(tbl2)

        if "Boxplot" in picked:
    ...
    # -------- 5.4 Output pack（作业写结论的时候很方便）
    with tabC:
        st.subheader("Task 5 output pack (quick results)")
        ...
        c1, c2 = st.columns(2)
        with c1:
            pack_cat = st.selectbox("Pack: categorical variable", cat_vars, key="t5_pack_cat") if cat_vars else None
            top_k = st.slider("Top K (categorical)", 5, 20, 10, 1, key="t5_pack_topk")
        with c2:
            pack_num = st.selectbox("Pack: numeric variable", num_vars, key="t5_pack_num") if num_vars else None
